...
| `KnowledgeBase` 新增迁移字段（`target_model`、`migration_status`、`migration_progress`、`locked_at`、`locked_by`、`locked_model`）；`TenantSystemConfig` 新增 `embedding_query_strategy` / `show_embedding_details` | F043-embedding-model-migration | 知识库 embedding 模型平滑迁移：系统默认模型变更不影响已有知识库；支持立即迁移 / 稍后迁移 / 锁定；迁移期间双模型 RRF 融合查询；新增迁移管理 API 与技术详情显示功能；不改动 `KnowledgeBase` 已有写入行为（model 字段仅由迁移流程更新） |
...
| F043-embedding-model-migration | F004, F008, F029, F030 | 知识库 embedding 模型平滑迁移；新增 `KnowledgeBase` 迁移字段 + Alembic；新增 `embedding_query_strategy` / `show_embedding_details` 配置；新增迁移管理 API（`/knowledge/{kb_id}/migration/*`）+ Embedding 配置 API（`/embedding-config`）；新增 Celery 任务驱动向量重建；新增 EmbeddingQueryRouter（含 RRF 融合）；新增前端迁移影响分析页 + 技术详情折叠面板；不新增领域对象、不改动既有 KB 写入行为；新增 140 段错误码 `EmbeddingMigration*`（14001–14019） |
