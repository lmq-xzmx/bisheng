...
from ..domain.services.embedding_config_service import EmbeddingConfigService
...
async def _read_before_snapshot(target_tenant_id: int, key: str) -> dict | None:
...
async def get_workbench_llm(request: Request, login_user: UserPayload = Depends(UserPayload.get_login_user)):
...
async def invoke_workbench_asr(
    request: Request, login_user: UserPayload = Depends(UserPayload.get_login_user), file: UploadFile = None
):
...
async def get_knowledge_llm(request: Request, login_user: UserPayload = Depends(UserPayload.get_login_user)):
...
# --- F050: Embedding query configuration -----------------------------------------


@router.get("/embedding-config")
async def get_embedding_config(login_user: UserPayload = Depends(UserPayload.get_login_user)):
    """Get embedding query configuration (strategy and show_details)."""
    config = await EmbeddingConfigService.get_embedding_config()
    return resp_200(data=config)


@router.put("/embedding-config")
async def update_embedding_config(
    strategy: str = Body(..., description="Query strategy: new_only, old_only, or dual_rrf"),
    show_details: bool = Body(..., description="Whether to show embedding details in chat"),
    login_user: UserPayload = Depends(UserPayload.get_tenant_admin_user),
):
    """Update embedding query configuration."""
    await EmbeddingConfigService.set_embedding_query_strategy(strategy)
    await EmbeddingConfigService.set_show_embedding_details(show_details)
    config = await EmbeddingConfigService.get_embedding_config()
    return resp_200(data=config)
